...
function modalAnexo(id) {
      idactividad = id;
    // $("#ModalAnexo .modal-content").load(base_url() + controlador + '/ModalAnexo', function () {
        $("#ModalAnexo").modal({
            backdrop: 'static',
            keyboard: true,
            show: true
        });
        obtenerAnexoActividad(id);
    // });
}
        ...
        success: function (resultado) {
            ...
            if (data.length > 0) {
                ...
                $.each(data, function () {
                    ...
                    let anexo = '<button class="modalAnexo btn btn-info btn-xs" title="Anexo"><span class="fas fa-upload"></span></button>';
                    ...
                    if (action) {
                        fila[fields.length + 1] = '<center>' + stateEdit + ' ' + edit +  ' ' + anexo + '</center>';
                    }
...
function loadIndc() {
    $.ajax({
        url: base_url() + controlador + "/consultarIndicador",
        type: "POST",
        async: false,
        success: function (resultado) {
            try {
			...
			success: function(resultado){
				...
				if (resultado == 3) {
					...
					if (resultado != 0) {
						...
						obtenerAnexoActividad(idactividad);
					}else{
