...
function dataLoad() {
    $.ajax({
        url: base_url() + controlador + "/consultarInscribir",
        type: "POST",
        success: function (resultado) {
            ...
            if (data.length > 0) {
                $.each(data, function () {
                    var  stateEdit = '<button class="stateEdit btn btn-success btn-xs" value="' + this.idactividad + '" title="Activar" style="margin-bottom:3px;margin: 0px 0px 0px 6px;"><span class="fas fa-sm fa-check"></span></button>';
                    var fila = {};
